...
if ($numrows != 0) {
	...
	$totalrow = mysql_fetch_array (mysql_query ("SELECT SUM(uniquehits) AS total FROM stats_daily WHERE SUBSTRING(subdate, 4, 2)='$month'"));
	
	$max = mysql_fetch_array (mysql_query ("SELECT MAX(uniquehits + 0) AS largest FROM stats_daily"));
...
while ($myrow = mysql_fetch_array ($result)) {
	...
	imagestring ($im, 4, 180, 255, "Daily Visits in $month_name", $line);
