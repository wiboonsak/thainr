...
<?php
	$query ="SELECT * FROM `tbl_rubber_cess`    WHERE cess_year  = '$CessYear '  AND cess_month ='$CessMonth' AND language='1' LIMIT 0,1  ";
	
	$resultData = mysql_query($query);
   //Create an XML data document in a string variable
   $strXML  = "";
   $strXML .= "<chart encoding='windows-874' caption='ราคาเกณฑ์ในการคำนวณค่า Cess'  xAxisName=''  yAxisName='' showValues='0'    formatNumberScale='0' showBorder='1' >";
while($data=mysql_fetch_assoc($resultData)){ 	
			
   $strXML .= "<set label='1-15 \n ราคายาง' value='".$data['1_15_price']."' />";
   $strXML .= "<set label='1-15 \n Cess' value='".$data['1_15_cess']."' />";
   $strXML .= "<set label='' value='' />";   
   $strXML .= "<set label='16-31 \n ราคายาง' value='".$data['16_31_price']."' />";
   $strXML .= "<set label='' value='' />";    
