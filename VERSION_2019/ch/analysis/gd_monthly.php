...
if ($numrows != 0) {
	...
	$totalrow = mysql_fetch_array (mysql_query ("SELECT SUM(hits) AS total FROM stats_monthly WHERE SUBSTRING(subdate, 7, 4)='$year'"));
	
	$max = mysql_fetch_array (mysql_query ("SELECT MAX(hits + 0) AS largest FROM stats_monthly"));
...
while ($myrow = mysql_fetch_array ($result)) {
	...
	imagestring ($im, 4, 180, 255, "Monthly Hits in $year", $line);
