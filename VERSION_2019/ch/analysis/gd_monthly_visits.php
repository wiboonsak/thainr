...
if ($numrows != 0) {
	...
	$totalrow = mysql_fetch_array (mysql_query ("SELECT SUM(uniquehits) AS total FROM stats_monthly WHERE SUBSTRING(subdate, 7, 4)='$year'"));
	
	$max = mysql_fetch_array (mysql_query ("SELECT MAX(uniquehits + 0) AS largest FROM stats_monthly"));
...
while ($myrow = mysql_fetch_array ($result)) {
		...
		if ($myrow['uniquehits'] != 0) {
			
			$height = round (225 - (($myrow['uniquehits'] / $highest) * 200));
		...
		$ximage = 40 + (38 * substr ($myrow['subdate'], 3, 2));
		imageline ($im, $ximage, 225, $ximage, 230, $line);

		if (substr ($myrow['subdate'], 3, 2) == date ("m")) {
		
			imagestring ($im, 2, $ximage - 5, 235, substr ($myrow['subdate'], 3, 2), $highlight);
			imagefilledrectangle ($im, $ximage - 8, $height, $ximage + 8, 225, $highlight);
			
		} else {
	
			imagestring ($im, 2, $ximage - 5, 235, substr ($myrow['subdate'], 3, 2), $line);
			imagefilledrectangle ($im, $ximage - 8, $height, $ximage + 8, 225, $bar);
		
		}

		if ($myrow['hits'] != 0) {
		
			imagestringup ($im, 2, $ximage - 7, $height - 5, $myrow['uniquehits'], $highlight);
			
		}

	}

	imagestring ($im, 4, 180, 255, "Monthly Visits in $year", $line);
