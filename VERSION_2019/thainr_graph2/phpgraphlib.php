<?php 
///////////////////////////////////////////////////////////
//PHPGraphLib -  PHP Graphing Library v2.01
...
class PHPGraphLib
{
			...
			{		
				...
				if($this->bool_bars)
				{
					...
					else
					{
						...
						if($y1<$y2){ imagefilledrectangle($this->image, $x1, $y1,$x2, $y2,  $this->multi_bar_colors[$data_set_num]); }
						else{ imagefilledrectangle($this->image, $x1, $y2,$x2, $y1,  $this->multi_bar_colors[$data_set_num]); }
					}
					//DRAW BAR OUTLINE	
	...
	function addData($data, $data2='',$data3='')
	{
	...
	{
		if($min<0||$max<=0){ $this->error[]="Range in setRange() must be non-negative."; }
		else if(is_numeric($max)&&is_numeric($min)&&$max>$min)
		{
			$this->data_range_max=$max;
			$this->data_range_min=$min;
			$this->bool_user_data_range=true;
		}
		else{ $this->error[]="Range in setRange() not specified properly. Consult documentation."; }
	}
	...
	function setLineColor($color, $color2='', $color3='')
	{
	...
	function setGradient($color1, $color2, $color3='', $color4='', $color5='', $color6='')
	{
	...
	function setLegendTitle($title, $title2='', $title3='')
	{
		if(!empty($title)){ 		
			$len=strlen($title);
