<?php 
	...
	if($_POST['action']=='Delete'){
			 $query="SELECT * FROM `tbl_rubber_statistic` WHERE  `id` =  '".$_POST['IDS']."' ";
			 //echo $query."<br>";
			  $resultPic=mysql_query($query);
			  $file=mysql_fetch_assoc($resultPic);
				 		@unlink($path_product.$file['file_th']);
						@unlink($path_product_thb.$file['file_th']);						 
			$query="DELETE FROM `tbl_rubber_statistic`  WHERE  `id` =  '".$_POST['IDS']."' ";
			mysql_query($query);	
			// echo $query."<br>";		 
		}
		
	###################################################
	  if($_POST['cate_id']==''){ $_POST['cate_id']='thai'; }
	####################################################	
	   if($_POST['cate_id']!=''){
				$query="SELECT * FROM   `tbl_rubber_statistic` WHERE  `statistic_type` ='".$_POST['cate_id']."'  ORDER BY id DESC";
...
<table width="99%" border="0" align="center" cellpadding="2" cellspacing="2">
  <tr class="red">
    ...
    <td width="96%" bgcolor="#D9D9B3">รายชื่อสถิติ เลือกหมวด

    
      <select name="cate_id" id="cate_id" onchange="document.form1.submit()">
        <option value="0">---- เลือกหมวดสถิติ-----</option>
        <option value="thai" <?php if($_POST['cate_id']=='thai') echo "selected";?>>สถิติยางพาราไทย</option>
        <option value="world" <?php if($_POST['cate_id']=='world') echo "selected";?>>สถิติยางพาราโลก</option>
        <option value="other" <?php if($_POST['cate_id']=='other') echo "selected";?>>สถิติอื่นๆ</option>
      </select>
  ...
  <tr>
    ...
    <table width="100%" border="0" cellpadding="2" cellspacing="2" class="txt10-black">
      ...
      <tr>
        ...
        <td align="center" bgcolor="#EFEFEF"><a href="main.php?work=<?php echo $work?>&currID=<?PHP echo $data['id']?>"><img src="images/black_icon/16x16/doc_edit.png" width="16" height="16" style="border:none" /></a></td>
