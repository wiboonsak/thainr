<?php 
		...
		//-----------------------------------------------------------------------------		
			if($_POST['action']=='deletePic'){
				 ...
				 $query = "DELETE FROM  tbl_situation_data_file WHERE  file_name = '".$_POST['RmPic']."' ";
				 mysql_query($query);
			}		
		//-----------------------------------------------------------------------------
		if($_POST['action']=='Add'){
				$_POST['date_act']=$_POST['syear']."-".$_POST['smonth']."-".$_POST['sdate'];
					if($_POST['currID']==''){
						 ...
						 $query="INSERT INTO `tbl_situation_data` "
							...
							$query="UPDATE tbl_situation_data SET "
						 ...
						 for($i=0;$i<count($_FILES["uploadfile"]["name"]);$i++)
							{
								...
								//------------------------------------------------
								$query=" INSERT INTO `tbl_situation_data_file` ( `id` , `w_th` , `w_en`, `file_name` , `news_id` , `file_type`, `origin_name_th`  ) "
														."	VALUES "
					...
					for($i=1;$i <$_POST['hiddenLoops'];$i++){
						     $query="UPDATE tbl_situation_data_file SET    `w_th`='".$_POST['wth'][$i]."'     , `w_en` ='".$_POST['wen'][$i]."'  WHERE id ='".$_POST['hiddenID'][$i]."'  ";	
		 ...
		 if($_POST['currID']){
			 $query="SELECT * FROM tbl_situation_data WHERE id =  '".$_POST['currID']."' ";		
			 ...
			 $currentData =mysql_fetch_assoc($result);
			  ...
			  $year=$dateArray[0];			  
			 //--------------select img--------------//
			 $query="SELECT * FROM `tbl_situation_data_file` WHERE  `news_id` =  '".$_POST['currID']."'  ORDER BY  file_type  ASC, id ASC";
...
<script language="javascript">
		function  checkForm(){
					if((document.form1.topic_th.value=='')){
...
<table width="99%" border="0" align="center" cellpadding="2" cellspacing="2">
  <tr class="red">
    ...
    <td width="96%" bgcolor="#D9D9B3">สถานการณ์ยางพารา</td>
  </tr>
  <tr>
    <td colspan="2"><table width="100%" border="0" cellpadding="2" cellspacing="2" class="txt10-black">
      <tr>
        <td width="13%" align="right"><strong><?php echo _txtTopicinput?></strong></td>
        <td width="87%"><input name="topic_th" type="text" id="topic_th" size="60" maxlength="255"  value="<?php echo $currentData['topic_th']?>"/></td>
      </tr>
      <tr>
