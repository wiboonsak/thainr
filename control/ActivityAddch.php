<?php 
		...
		//-----------------------------------------------------------------------------		
			if($_POST['action']=='deletePic'){
				 ...
				 $query = "DELETE FROM  tbl_news_and_act_file WHERE  file_name = '".$_POST['RmPic']."' ";
				 mysql_query($query);
			}		
		//-----------------------------------------------------------------------------
		if($_POST['action']=='Add'){
			$_POST['date_act']=$_POST['syear']."-".$_POST['smonth']."-".$_POST['sdate'];
					if($_POST['currID']==''){
						 ...
						 $query="INSERT INTO `tbl_news_and_act` "
										    ." ( `id` , `topic_th` , `topic_en` , `topic_ch` , `detail_th` , `detail_en` , `detail_ch` , `date_act`, `cate_id` , `language` ) "
											." VALUES "
											." ('' , '', '','".$_POST['topic_ch']."', '', '','".$_POST['detail_ch']."',  '".$_POST['date_act']."' ,  '".$_POST['cate_id']."' ,  '".$_SESSION['language']."') ";
								mysql_query($query);
								$_POST['currID'] =mysql_insert_id();
						}else{
							$query="UPDATE tbl_news_and_act SET "
							." `topic_ch`='".$_POST['topic_ch']."' ,`detail_ch`='".$_POST['detail_ch']."'  , `date_act`='".$_POST['date_act']."' , `cate_id` = '".$_POST['cate_id']."'  "
						 ...
						 for($i=0;$i<count($_FILES["uploadfile"]["name"]);$i++)
							{
								...
								//------------------------------------------------
								$query=" INSERT INTO `tbl_news_and_act_file` ( `id` , `w_th` , `w_en`, `file_name` , `news_id` , `file_type`, `origin_name_th` , `picSort`  ) "
														."	VALUES "
					...
					for($i=1;$i <$_POST['hiddenLoops'];$i++){
						     $query="UPDATE tbl_news_and_act_file SET `w_th`='".$_POST['wth'][$i]."'     , `w_en` ='".$_POST['wen'][$i]."'  WHERE id ='".$_POST['hiddenID'][$i]."'  ";	
							 mysql_query($query);  
							 
							 $queryFile="UPDATE tbl_news_and_act_file SET  `picSort`='".$_POST['sort'][$i]."' WHERE id ='".$_POST['hiddenID'][$i]."'  ";	
							 mysql_query($queryFile);
						}
			}
		
		 if($_POST['currID']){
			 $query="SELECT * FROM tbl_news_and_act WHERE id =  '".$_POST['currID']."' ";		
			 ...
			 $dateArray=explode("-",$currentData['date_act']);
			  ...
			  $year=$dateArray[0];		
			 //--------------select img--------------//
			 $query="SELECT * FROM `tbl_news_and_act_file` WHERE  `news_id` =  '".$_POST['currID']."'  ORDER BY   picSort  ASC, id ASC , file_type  ASC ";
...
<table width="99%" border="0" align="center" cellpadding="2" cellspacing="2">
  <tr class="red">
    ...
    <td width="96%" bgcolor="#D9D9B3">เพิ่มข่าวสารกิจกรรม
  ...
  <tr>
    <td colspan="2"><table width="100%" border="0" cellpadding="2" cellspacing="2" class="txt10-black">
      <tr>
        <td width="14%" align="right"><strong><?php echo _txtSelectCagegory?></strong></td>
        <td width="86%"><select name="cate_id" id="cate_id">
          <option value="0">---- <?php echo _txtSelectCagegory?>-----</option>
          <option value="1" <?php if($currentData['cate_id']=='1') echo "selected";?>><?php echo _txtTraOther?></option>
          <option value="2" <?php if($currentData['cate_id']=='2') echo "selected";?>><?php echo _txtTraMeeting?></option>
        </select></td>
      </tr>
      <tr>
        <td align="right"><strong><?php echo _txtTopicinput?></strong></td>
        <td><input name="topic_ch" type="text" id="topic_ch" size="60" maxlength="255"  value="<?php echo $currentData['topic_ch']?>"/></td>
      </tr>
      <tr>
        <td align="right"><strong><?php echo _txtDetaulinput?></strong></td>
        <td><textarea name="detail_ch" cols="40" rows="5" id="detail_ch"><?php echo $currentData['detail_ch']?></textarea><script language="JavaScript1.2" type="text/javascript">generate_wysiwyg('detail_ch');
		          </script></td>
      </tr>
      <tr>
        ...
        <table width="" border="0" cellpadding="0" cellspacing="0" bgcolor="" >
                        <tr>
                          <td width="16"><?php  if($_POST['currID']){ ?>
                              <table width="" border="0" cellpadding="5" cellspacing="5" style="padding-top: 15px;">
                                <tr>
                                  <?php 
    		$col=1;$n=1;$num=1;
			$n_width=125;$n_height=90;
			$max_x =200;
			$max_y =200;
				while($pic = mysql_fetch_assoc($resultPic)){
						...
						if(($info[extension]=='pjpeg')  || ($info[extension]=='gif')  ||  ($info[extension]=='png')  ||  ($info[extension]=='x-png') || ($info[extension]=='jpeg') || ($info[extension]=='jpg')){
							...
							echo "</a></td> ";
							echo "<td><br>ลำดับภาพ <br> <input type='text' size='30' style='text-align:center;' name='sort[".$num."]' value='".$pic['picSort']."'></td>";
							echo "</td> ";
					if($n==$col){ echo "</tr>\n"; $n=0;};	
					 $n++; $num++;
				}
...
<!--        <input type="submit" name="button2" id="button2" value="<?php echo _txtBtnAddEdit?>" /></td>-->
        </tr>
    </table></td>
  </tr>
  <tr>
