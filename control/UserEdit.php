<?php 
	...
	if($_POST['action']=='Delete'){
			$query = "DELETE  FROM tbl_member_data WHERE id =  '".$_POST['IDS']."'  ";
			mysql_query($query);
	}
	####################################################
	if($_POST['action']=='addsort'){
			$query = "UPDATE `tbl_member_data` SET `sort_number`= '".$_POST['sortnumber']."' WHERE `id`= '".$_POST['IDS']."' ";
			mysql_query($query);
            //echo $query;
	}
	if($_SESSION['language']!='1'){
	$query="SELECT * FROM tbl_member_data WHERE  `language`= '".$_SESSION['language']."'  ORDER BY name ASC ";
        }else{
        $query="SELECT * FROM tbl_member_data WHERE  `language`= '1'  ORDER BY sort_number ASC ";
        }
...
<table width="99%" border="0" align="center" cellpadding="2" cellspacing="2">
  <tr class="red">
    ...
    <td width="96%" bgcolor="#D9D9B3">ข้อมูล Member ทั้งหมด</td>
  </tr>
  <tr>
    <td colspan="2"><table width="100%" border="0" cellpadding="1" cellspacing="1" class="cat_desc">
      ...
      <tr class="txt10-black">
        ...
        <td width="73%">&nbsp;<?php echo $data['name']?></td>
           <?php if($_SESSION['language']=='1'){?>
        <td width="8%" align="center" ><input style="width:80%;text-align: center" type="text" value="<?php echo $data['sort_number']?>" placeholder="เรียงลำดับ" onChange="form2.action.value='addsort'; form2.IDS.value='<?php echo $data['id']?>';form2.sortnumber.value=this.value;form2.submit();"/></td>
           <?php }?>
        <td width="8%" align="center" bgcolor="#FBFFE6"><a href="main.php?work=UserAdd&IDS=<?php echo $data['id']?>" >แก้ไข</a></td>
